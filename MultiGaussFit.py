"""
MultiGaussFit v3.0 — GUI Edition

Signal preprocessing and multi-Gaussian deconvolution tool with graphical interface.

Features:
  - Load CSV files with XYYY or XYXY data structures
  - Interactive signal selection and visualization
  - Signal preprocessing: baseline correction, filtering (SG/FT), minimum shift, negative removal
  - Click-on-plot interval selection for deconvolution
  - Multi-Gaussian deconvolution with 1 or 2 Gaussians per peak
  - SSE fit quality metric
  - Undo/Reset preprocessing steps
  - Export results to CSV

Author: Juan E. Lopez Cervantes
Repository: https://github.com/1JELC1
"""
...
def optimize_gaussians(x, y, centers, intervals, num_gaussians=1,
    ...
    """Optimize Gaussian parameters via differential evolution. Returns (params, fun)."""
...
class MultiGaussFitApp(tk.Tk):
    ...
    def __init__(self):
        ...
        try:
            import ctypes
            myappid = 'jelc.multigaussfit.gui.3.1'
    ...
    def _build_status_bar(self):
        self.status_bar = tk.Label(
            self, text="  Welcome to MultiGaussFit v3.0  —  Open a CSV file to begin",
    ...
    def _on_listbox_enter(self, event):
        """When cursor enters the signal listbox, scroll only the listbox."""
        self.bind_all('<MouseWheel>', self._listbox_scroll)

    def _on_listbox_leave(self, event):
        """When cursor leaves the listbox, revert to sidebar scrolling."""
        self.bind_all('<MouseWheel>', self._sb_scroll)

    def _on_results_enter(self, event):
        """When cursor enters the results text, scroll only the text widget."""
        self.bind_all('<MouseWheel>', self._results_scroll)

    def _on_results_leave(self, event):
        """When cursor leaves the results text, revert to sidebar scrolling."""
        self.bind_all('<MouseWheel>', self._sb_scroll)
    ...
    def _push_undo(self):
        """Save current y for undo; clear deconvolution results."""
    ...
    def _show_param_dialog(self, title, fields, callback):
        """Generic popup with labeled entry fields and an Apply button."""
    ...
    def _show_results(self):
        """Populate the results text widget with peak parameters."""
    ...
    def _run_batch_deconvolution(self):
        """Apply the same preprocessing + deconvolution to all remaining signals."""
        if self.deconv_params is None:
            messagebox.showinfo(
                "No Configuration",
    ...
    def _batch_worker(self, remaining, pipeline, intervals, n_gauss):
        """Process each remaining signal in sequence."""
    ...
    def _batch_show_preproc(self, idx, x, y, y_orig, name):
        """Display preprocessing result for a batch signal."""
        self.ax.clear()
        self._style_axes()
    ...
    def _batch_show_deconv(self, idx, x, y, y_orig, name,
        ...
        """Display deconvolution result for a batch signal."""
        nc = len(intervals)
        ...
        if has_deconv:
            ...
            if fp_dec:
                try:
                    ...
                    pd.DataFrame(rows).to_csv(fp_dec, index=False)
